...
// On Render, DATABASE_PATH points to the persistent disk mount; fallback to local for dev
const DB_FILE = process.env.DATABASE_PATH || path.join(ROOT, "data", "db", "netra.sqlite");
...
function seedDemoUsers() {
  ...
  const insertUser = db.prepare(`
    ...
    VALUES (@id, @name, @email, @password_hash, @created_at, @xp, @streak, @role, @preferred_language, @active_path_id, @onboarded)
  `);
  ...
  const seedTx = db.transaction(() => {
    demoNames.forEach((name, idx) => {
      ...
      insertUser.run({
        ...
        active_path_id: state.profile.activePathId,
        onboarded: 1
      });
      insertState.run({
        user_id: userId,
...
function createDbSession(userId) {
...
function lookupDbSession(token) {
...
async function registerUser(name, email, password) {
  ...
  const existing = getUserByEmail(emailKey);
  ...
  db.prepare(`
    INSERT INTO users (id, name, email, password_hash, created_at, xp, streak, role, preferred_language, active_path_id, onboarded)
    VALUES (?, ?, ?, ?, ?, 0, 0, ?, ?, ?, 0)
  `).run(userId, name, emailKey, simpleHash(password), now, "Student", "English", "grades-6-8");
...
async function loginUser(email, password) {
  const emailKey = email.toLowerCase().trim();
  const account = getUserByEmail(emailKey);
...
function applyUserProfile(state, userRow) {
  ...
  state.profile.id = userRow.id || state.profile.id;
  ...
  state.profile.xp = Number.isFinite(userRow.xp) ? userRow.xp : state.profile.xp;
  state.profile.streak = Number.isFinite(userRow.streak) ? userRow.streak : state.profile.streak;
  state.profile.onboarded = userRow.onboarded ? true : state.profile.onboarded;
...
function upsertUserFromProfile(userId, profile) {
  ...
  db.prepare(`
    ...
    VALUES (@id, @name, @email, @password_hash, @created_at, @xp, @streak, @avatar_data_url, @role, @preferred_language, @active_path_id, @onboarded, @last_activity_date)
...
async function writeState(userId, state) {
  ...
  `).run(payload);
  upsertUserFromProfile(userId, state.profile || {});
}
...
async function getLeaderboard(res, url, userId) {
  ...
  const rows = db.prepare("SELECT id, name, xp, streak FROM users ORDER BY xp DESC, streak DESC, created_at ASC").all();
...
async function handleSignup(req, res) {
  ...
  const token = createDbSession(result.userId);
...
async function handleGoogleAuth(req, res) {
  ...
  const emailKey = email.toLowerCase().trim();
  let account = getUserByEmail(emailKey);
  if (!account) {
    const userId = "goog-" + googleSub;
    const now    = new Date().toISOString();
    db.prepare(`
      INSERT OR IGNORE INTO users (id, name, email, password_hash, created_at, xp, streak, role, preferred_language, active_path_id, onboarded)
      VALUES (?, ?, ?, NULL, ?, 0, 0, 'Student', 'English', 'grades-6-8', 0)
    `).run(userId, name || emailKey.split("@")[0], emailKey, now);
    const initial = createInitialState();
    initial.profile.name = name || emailKey.split("@")[0];
    await writeState(userId, initial);
    account = getUserByEmail(emailKey);
  }
  const token = createDbSession(account.id);
  setSessionCookie(res, token);
